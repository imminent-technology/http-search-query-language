...
**Status: 60 of 60 languages evaluated.** (pilot batch + Relational/SQL-family batch + Document/NoSQL batch + Search/full-text & Graph batch + Analytics/Observability & JSON Query Language batch + Language-integrated/Log-security-search/Scripting/Path-document-navigation/Niche-misc/API-data-fetching-remainder batch + JSON Query Language (christosgkoros) name-collision addition + OData addition + JSONPath addition + JMESPath addition + RSQL/FIQL addition + SCIM Filter addition + PartiQL addition + CEL addition + LogQL/TraceQL addition + JQL/CQL (Confluence)/SOSL/Stripe Search Query Syntax/GitHub Search Syntax addition + Elasticsearch/OpenSearch Query DSL/Algolia/Meilisearch/Typesense/RediSearch addition). Note: an earlier miscount stated the project total as 39 languages; a full recount of `list.csv` confirmed the true total is 40, and it has since grown to 60 with the addition of a second, unrelated "JSON Query Language" spec, OData, JSONPath, JMESPath, RSQL/FIQL, SCIM Filter, PartiQL, CEL, and 12 further languages spanning observability (LogQL, TraceQL) and search/full-text (JQL, CQL (Confluence), SOSL, Stripe Search Query Syntax, GitHub Search Syntax, Elasticsearch/OpenSearch Query DSL, Algolia Filter Syntax, Meilisearch Filter Syntax, Typesense Filter Syntax, RediSearch Query Syntax).
